...
class Hoosk_model extends CI_Model {
    ...
    function checkRightmoveStatus() {
          $this->db->select("*");
          $this->db->where("siteID", 0);
            $query = $this->db->get('hoosk_settings');
          if ($query->num_rows() > 0) {
              $results = $query->result_array();
              if ($results[0]['rightmoveSendProperties']==1){
                return true;
              } else {
                return false;
              }
      ...
      function checkRightmoveLive() {
            $this->db->select("*");
            $this->db->where("siteID", 0);
              $query = $this->db->get('hoosk_settings');
            if ($query->num_rows() > 0) {
                $results = $query->result_array();
                if ($results[0]['rightmoveUseLive']==1){
                  return true;
                } else {
                  return false;
                }
        ...
        function getNetworkID() {
              ...
              if ($query->num_rows() > 0) {
                  $results = $query->result_array();
                	return $results[0]['rightmoveNetworkID'];
      		      }
          }
          function getPEMKEY() {
                $this->db->select("*");
                ...
                if ($query->num_rows() > 0) {
                    $results = $query->result_array();
                  	return $results[0]['rightmovePEMKEY'];
        		      }
            }
            function getPEMFile() {
                  $this->db->select("*");
                  ...
                  if ($query->num_rows() > 0) {
                      $results = $query->result_array();
                    	return $results[0]['rightmovePEMFile'];
          		      }
              }
	function getLang() {
        $this->db->select("*");
    ...
    'enableSearch' => $this->input->post('enableSearch'),
       	);
	...
	function updateSettings($rightmovePEMFile="") {
		$data = array(
			'siteTheme' => $this->input->post('siteTheme'),
			'siteLang' => $this->input->post('siteLang'),
			'siteFooter' => $this->input->post('siteFooter'),
      'rightmoveNetworkID' => $this->input->post('rightmoveNetworkID'),
			'rightmovePEMKEY' => $this->input->post('rightmovePEMKEY'),
      'rightmoveSendProperties' => $this->input->post('rightmoveSendProperties'),
			'rightmoveUseLive' => $this->input->post('rightmoveUseLive')
		);
    if ($rightmovePEMFile!= ""){
      $data['rightmovePEMFile'] = $rightmovePEMFile;
    }
